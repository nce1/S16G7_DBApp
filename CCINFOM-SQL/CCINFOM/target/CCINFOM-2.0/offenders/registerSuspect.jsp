...
<html>
    ...
    <body>
        <form action="processOffender.jsp" method="get" onsubmit="return validateInteger()">
            <jsp:useBean id="cas" class="ccinfom.database.Case" scope="session" />
            <h2>Register Suspect</h2>

            <label for="id">ID:</label>
            <input type="number" id="id" name="id" required>
            
            <label for="offense">Offense:</label>
            <input type="text" id="offense" name="offense" maxlength="50" required>
            
            <label for="caseId">Open Cases:</label>
            <select name="caseId" id="caseId">
            <option value="null"> <%= "Select" %> </option>
            <%  
                cas.getActiveCaseList();

                for (int i = 0; i < cas.allCaseIdList.size(); i++) { 
            %>
                <option value="<%= cas.allCaseIdList.get(i) %>"> <%= cas.allCaseIdList.get(i) %> </option>
            <% 
                }
            %>
            </select>
